...
| P1.9 | ✅ **FATTO** (2026-09-16, `docs/22` §3; **corretto** lo stesso giorno in `docs/23` §3 — il gate in CI ha trovato due difetti: la riga sospesa contava le richieste dello scoreboard e una sonda fallita riapriva la fonte, 5× il costo dichiarato) — ESPN 403 da **76 run consecutivi** (14 richieste/run): `src/fda/backoff.py` con stato derivato da `source_status`, pausa e sonda ogni 4 run, pill **SOSPESO** in *Stato fonti*, invariante `[28]` estesa | `backoff.py`, `collect.py`, `status.html` | basso |
