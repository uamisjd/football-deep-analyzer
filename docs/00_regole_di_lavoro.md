...
**PR che cambiano il modello di produzione — la griglia si dichiara nel codice prima del run (regola del 2026-09-18, P1.11 — `docs/19` §1.4).** Un candidato del laboratorio che cerca un iperparametro dichiara la griglia dei valori provati **nel codice** (`Candidate.grid` in `src/fda/models/lab.py`), prima di guardare i risultati; griglia dichiarata e numero di tentativi della stessa famiglia finiscono in `model_lab.parquet` (colonne `grid_dichiarata` e `n_tentativi`) e `scripts/verdetto_lab.py` li stampa accanto al verdetto. Se la griglia cambia, il run precedente resta in `model_lab.parquet` con la sua griglia: **il confronto onesto è fra griglie dichiarate, non fra risultati scelti**. Motivo misurato: lo stesso γ-sharpening *passa* i criteri di promozione con griglia [0,94; 1,12] e *non passa* con [1,00; 1,40] (`docs/19` §1.3). Nessuna promozione — e quindi nessuna PR che cambi `predict.py` — senza griglia dichiarata.

**PR ricca di sessione (direttiva utente 2026-09-18: «più cose possibile senza commettere errori prima di fare il merge»).** Per aumentare quanto ogni sessione porta in `main` senza accumulare rischio, la sessione accumula sul proprio branch `arena/...` **più interventi della coda**, ciascuno chiuso nel modo già prescritto (committato e **pushato subito**, regole A1/A8/8bis; test tematici + `ruff` pulito sul codice nuovo), e apre **una sola PR a fine sessione** solo dopo i gate pieni: suite completa verde, `ruff` = baseline, `fda build` exit 0 e `verify_site` 0 problemi (questi ultimi due obbligatori se il sito è toccato). Se un gate è rosso la PR non si apre: si corregge o si sfila il pezzo che non passa. **Restano fuori dalle PR ricche** (PR propria, come prima): (a) modifiche alla **ricetta di produzione** (`predict.py`, `calibration.py`, ensemble) — cambiano ogni numero pubblicato e restano sotto protocollo del laboratorio + griglia pre-registrata; (b) **pulizia meccanica larga** (es. ruff `--fix` di massa su 100 punti): il diff enorme nasconderebbe i contenuti veri; (c) **nuovi workflow/fonti** che si possono verificare solo dal vivo dopo il merge; (d) ciò che l'utente chiede separato. La regola A2 non cambia: i turni restano piccoli e conclusi — è la PR a diventare più densa, non il turno.
