...
scaricati (188 commit, `git cat-file --batch-all-objects`; nessuno inizia per `f2eb01c`, e
...
- Dati rigenerati con la nuova ricetta: `backtest.parquet` (5.811 gare), `calibration.parquet`,
  `predictions.parquet` (2.071 partite).

## 3. Misure prima/dopo **sullo stesso corpus** (5.811 gare fuori campione, 7 leghe)

Confronto appaiato: stessa partita, stessa finestra, stesso storico; l'unica differenza è la
ricetta. La vecchia griglia è stata ricalcolata con la ricetta precedente (`inverti`) sullo
stesso `history.parquet`.

| metrica | grezzo vecchio | grezzo nuovo | calibrato vecchio | calibrato nuovo |
|---|---|---|---|---|
| RPS 1X2 | 0,19919 | 0,19898 | 0,19958 | **0,19882** |
| log-loss | 0,98637 | 0,98464 | 0,98630 | **0,98393** |
| Brier 1X2 | 0,58721 | 0,58674 | 0,58771 | **0,58636** |
| Brier 6 mercati | 0,20578 | 0,20589 | 0,20460 | 0,20507 |
| gol attesi (osservati 2,863) | 3,104 | 2,730 | 2,835 | 2,839 |
| bias λ | **+0,240** | −0,134 | −0,028 | −0,024 |
| pareggio previsto (osservato 25,6%) | 23,9% | **25,7%** | 26,2% | 25,9% |
| Over 2,5 previsto (osservato 54,9%) | 58,9% | 50,7% | 53,1% | 53,2% |

Bootstrap appaiato a 95% (4.000 ricampionamenti, 5.811 gare) sul **calibrato**:

| metrica | Δ (nuovo − vecchio) | IC 95% | leghe a favore |
|---|---|---|---|
| RPS 1X2 | **−0,00077** | [−0,00101; −0,00051] | **7 su 7** |
| Brier 1X2 | **−0,00135** | [−0,00193; −0,00077] | — |
| Brier 6 mercati | +0,00047 | [−0,00015; +0,00105] | 0 su 7 |
| obiettivo `fda calibrate` (RPS + 0,5·Brier) | **−0,00053** | [−0,00094; −0,00012] | — |

Lettura onesta:

1. **l'1X2 migliora in modo significativo e in tutte e 7 le leghe** (−0,00077 di RPS, intervallo
   interamente negativo), e il log-loss migliora il triplo (−0,0024);
2. **il Brier dei mercati sui gol peggiora di 0,00047, con un intervallo che contiene lo 0**:
   ...
   distribuzione: con λ più basse gli Over perdono un po' di massa (Over 1,5 +0,00087,
   Over 2,5 +0,00114, Over 3,5 +0,00043, BTTS +0,00044; porte inviolate −0,00026/+0,00020).
   È il prezzo dichiarato della promozione, ed è più piccolo del guadagno sull'obiettivo che
   `fda calibrate` ottimizza (−0,00053, intervallo interamente negativo);
3. **il pareggio grezzo è ora centrato** (25,7% contro 25,6% osservato, scarto 0,05 punti) senza
   bisogno di correzioni: era il difetto più visibile (23,9% contro 25,6%).

Verifica walk-forward della calibrazione (4.842 gare tenute fuori, stessi parametri per entrambe
le ricette):

| | RPS prima → dopo | Brier mercati prima → dopo | bias λ prima → dopo |
|---|---|---|---|
| ricetta precedente | 0,20025 → 0,20050 (+0,00025) | 0,20648 → 0,20518 (−0,00129) | +0,261 → +0,053 |
| **ricetta promossa** | 0,20009 → **0,19993 (−0,00016)** | 0,20637 → 0,20566 (−0,00071) | −0,113 → **+0,019** |
...
|---|---|
| bias λ **−0,370** gol | bias λ **−0,024** gol |
| Brier 6 mercati **0,21017** (contro 0,20460) | Brier 6 mercati 0,20507 |
...
(`fda calibrate`, stimatore a momenti, finestra 730 giorni, 4.759 gare di stima su 5.811):

```
λ×1,0401 · ρ−0,04 · cal-momenti-1.1
walk-forward (4.842 gare): RPS 0,20009 → 0,19993 · Brier mercati 0,20637 → 0,20566
                           bias λ −0,113 → +0,019 gol
campione pieno:            bias λ −0,134 → −0,024 · pareggio 25,7% → 25,9% (osservato 25,6%)
```

Il moltiplicatore torna sopra 1 — e sotto il limite di sicurezza 1,05 — proprio perché il tilt
ha smesso di gonfiare le λ: ora la correzione tira i gol attesi **verso l'alto**, non verso il
basso. È la firma del difetto che la promozione rimuove.
...
- suite **176 passed** (3 test nuovi sulla ricetta promossa, 2 aggiornati), `ruff --select F,E9`
  pulito su `src`, `tests`, `scripts`;
...
- `scripts/verify_site.py`: **0 problemi · 12.081 controlli numerici**, 4.115 pagine (375
...
(Over 1,5 +0,00087, Over 2,5 +0,00114). La causa strutturale è nota in letteratura
