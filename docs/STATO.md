...
**Ultimo aggiornamento:** 2026-09-08 10:32 UTC (controllo live NED1/POR1: ancora **nessun `daily` post-merge partito** — ultimo run `34206369724` creato 08:46:56Z su `a167b56` pre-merge; slot 10:00 non ancora partito, ritardi cron fino a ~5 h. Riapplicata policy utente «il merge lo esegue SEMPRE l'utente» su `00`/`BRIEFING`/`STATO` — era andata persa col commit locale `9d39a21` mai pushato. Sessione: branch `arena/01a08091-football-deep-analyzer`.)

## Fatto
- [x] **Processo/handoff — policy «merge lo esegue SEMPRE l'utente»** (decisione utente 2026-09-08, riapplicata in questa sessione perché il commit originale `9d39a21` non era mai stato pushato): sez. D di `00_regole_di_lavoro.md` riscritta (dopo la frase fissa l'agente si ferma e aspetta, non mergea mai), avviso ⚠️ in cima a `BRIEFING_NUOVA_SESSIONE.md`, decisione registrata in «Decisioni aperte». Doc-only; test 23 passed, nessuna nuova segnalazione ruff (baseline: vedi Nota). **PR #9 aperta verso `main`** (in attesa check/merge utente).
...
7. ~~Upgrade Node 24~~ ✅
7b. storico NED1/POR1 (merge PR #4 in `main` ✅ 09:23:46 UTC) → **confermare NED1/POR1 sul primo `daily` di `main` post-merge** (verifica stato rifatta il 2026-09-08 10:32 UTC: nessun daily post-merge ancora partito, slot 10:00 in ritardo; gare NED1 già oggi e POR1 da domani nei dati → attesa breve), poi report pre/post in italiano e rifinitura di Accuratezza con le prime gare reali (già 19 gare valutate).
