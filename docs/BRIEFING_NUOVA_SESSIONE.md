...
| `docs/13_modelli_e_schede_piano_2026-09-13.md` | Diagnosi misurata dei modelli (bias dei gol), calibrazione della griglia, laboratorio walk-forward, ricerca esterna, nuove viste della scheda, criteri di accettazione e P2/P3. |
...
| `.github/workflows/daily.yml` | Cron 5x/giorno + dispatch manuale → `fda daily` (collect → **calibrate** → predict → backtest → simulate → build), commit dati, Pages. |
| `.github/workflows/lab.yml` | Laboratorio modelli: lunedì 05:30 IT + dispatch, su `history.parquet` salvato dai run daily (finestre 90 gg). |
