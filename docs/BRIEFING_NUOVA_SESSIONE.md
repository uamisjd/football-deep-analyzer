...
9. **Dopo il merge di questa sessione** (`docs/13` §6): primo run `daily` → confermare `n_fit ≥ 1.200`, `bias_lambda < 0,10`, `history.parquet` presente, pareggio dentro l'intervallo di Wilson in Accuratezza; poi dispatch del workflow `lab` su storico reale e **cambio di modello solo se** un candidato batte `dc_elo_prod` con IC 95% appaiato interamente negativo e vince in ≥5 leghe su 7. A seguire il P2: xG storici cumulati (candidato `dc_xg`), ξ per lega, tabella del laboratorio pubblicata in Accuratezza, dotplot dei punti nelle Proiezioni.
