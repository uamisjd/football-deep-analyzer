...
- **Sessione corrente `arena/01a09a79-football-deep-analyzer` (2026-09-13, secondo giro):** precisione dei modelli e leggibilità delle schede — piano, ricerca e misure in **`docs/13_modelli_e_schede_piano_2026-09-13.md`**. (1) **Calibrazione della griglia** (`fda calibrate`, tabella `calibration`): sul backtest fuori campione (5.791 gare, 7 leghe) il modello gonfiava i gol attesi (**λ 3,107 vs 2,862 osservati, +8,6% in tutte le leghe**) perché `ensemble()` ricalcola le λ con `goal_expectancy` sull'1X2 mescolato con Elo; correggendo **λ×0,94 e Δρ=−0,04** e ripubblicando tutta la previsione dalla griglia corretta il bias scende a **+0,058**, il pareggio dichiarato passa da 23,9% a **25,7%** (osservato 25,6%) e il Brier mercati walk-forward da 0,2065 a **0,2055**; l'RPS 1X2 resta 0,1993 (invariato entro il rumore: **l'1X2 è al pavimento informativo**, per migliorarlo serve informazione nuova). `fda daily` ora è collect → **calibrate** → predict → backtest → simulate → build. (2) **Laboratorio modelli** (`fda lab`, tabella `model_lab`, workflow `lab.yml` il lunedì 05:30 IT): **15 candidati** (DC e varianti ξ/shrinkage, Poisson, bivariata, binomiale negativa, zero-inflazionata, copula di Weibull, Elo, Pi-ratings, miscele nella griglia 50/85) in walk-forward con **bootstrap appaiato al 95%**; gira su `history.parquet`, che da ora `fda simulate` salva a ogni run (prima lo storico veniva riscaricato e non restava traccia di su che cosa fosse allenata ogni previsione). Primo giro dal sandbox solo su corpus **surrogato** da h2h (distorto, −10% di gol): nessun candidato significativamente diverso dalla baseline → **la scelta va presa sul primo giro in Actions**. (3) **Schede**: blocchi «Quanti gol, in pratica» (distribuzione dei gol totali in partite su 100 + **dotplot quantile a 20 punti**) e «Come nasce questa probabilità» (catena DC → Elo → calibrazione con Δ pp); `verify_site.py` controlli **[9]** e **[10]**; suite **150 passed**; **4.088 pagine · 0 problemi**.
